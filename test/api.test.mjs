...
test('HTTP API registers, authenticates, isolates saves, and validates economy actions', async () => {
  ...
  try {
    ...
    const firstBoss = await request('/api/boss-clear', { method: 'POST', cookie, body: { bossId: 'zigou' } });
    assert.equal(firstBoss.status, 200);
    assert.equal((await firstBoss.json()).result.letter, true);
    for (let index = 0; index < 2; index++) assert.equal((await request('/api/boss-clear', { method: 'POST', cookie, body: { bossId: 'zigou' } })).status, 200);
